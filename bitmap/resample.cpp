...
public:
	static inline int ActualX( int x, const ResampleInfo_t &info )
	{
		...
		const int result = x % info.m_nSrcWidth;
		if ( result < 0 )
		{
			return info.m_nSrcWidth + result;
		}
		else
		{
			return result;
	...
	static inline int ActualY( int y, const ResampleInfo_t &info )
	{
		...
		const int result = y % info.m_nSrcHeight;
		if ( result < 0 )
		{
			return info.m_nSrcHeight + result;
		}
		else
		{
			return result;
	...
	static inline int ActualZ( int z, const ResampleInfo_t &info )
	{
		...
		const int result = z % info.m_nSrcDepth;
		if ( result < 0 )
		{
			return info.m_nSrcDepth + result;
		}
		else
		{
			return result;
