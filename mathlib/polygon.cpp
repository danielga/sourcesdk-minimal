	...
	for ( int i = 0; i < 2; ++ i )
	{
		...
		if ( pNewPartition->m_Indices.Count() >= 3 )
		{
			...
			if ( nHoleIndex == -1 )
			{
				...
				for ( int iHole = 0; iHole < pHoles->Count(); ++iHole )
				{
					vHolePoint = SubPolygon_t::GetPoint( polygonPoints, pHoles->Element( iHole ).m_Indices[0] );
					if ( IsPointInPolygon( polygonPoints, *pNewPartition, vNormal, vHolePoint ) )
					{
						// hole is within the region
						nHoleInRegionIndex = iHole;
						break;
					}
				}
			...
			if ( nHoleIndex != -1 )
			{
				...
				for ( int iHolePoint = 0; iHolePoint < pHolePolygon->m_Indices.Count(); ++iHolePoint )
				{
					pOriginalPolygon->m_Indices.InsertAfter( nInsertAfterIndex, pHolePolygon->m_Indices[( iHolePoint + nHoleVertexIndex ) % pHolePolygon->m_Indices.Count()] );
					++ nInsertAfterIndex;
				}
			...
			else
			{
				...
				for ( int iPartitionPoint = 1; iPartitionPoint < pNewPartition->m_Indices.Count() - 1; ++iPartitionPoint )
				{
					nIndexToRemove = nIndexToRemove % pOriginalPolygon->m_Indices.Count();
					Assert( pOriginalPolygon->m_Indices[nIndexToRemove] == pNewPartition->m_Indices[iPartitionPoint] );
