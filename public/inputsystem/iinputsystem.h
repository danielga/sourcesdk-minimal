...
public:
	...
	// System IME abstraction.  Note that this is separate from
	// specific IME implementations such as those in scaleform
	// and panorama.  Which one is active or used depends on
	// the context of the input.

	// Allow or disallow IME activation. This is not an overall enable
	// of the IME system, it is an input mode. For example when a game
	// is being played and no text input is desired the IME can be
	// disallowed to avoid it popping up from keystrokes used for the game.
	// Note that multiple components may have IME involvement.
	// This only affects base inputsystem IME state.
	virtual bool IsIMEAllowed() const = 0;
	virtual void SetIMEAllowed( bool bAllowed ) = 0;

	// Hint at the location of text input UI that
	// cares about possible IME UI.
	virtual void SetIMETextLocation( int x, int y, int nWidth, int nHeight ) = 0;

	// If the IME is active dismiss any current UI.
	// Note that multiple components may have IME involvement.
	// This only affects base inputsystem IME state.
