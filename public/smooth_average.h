...
inline CTimingInfo< T > CalcSmoothAverage_Struct( const T &value, int nTimes, const char *pFilename, int iLine )
{
	...
	for ( int i=1; i < pInfo->m_Values.Count(); i++ )
	{
		if ( i != newValueIndex )
		{
			info.m_HighAverage = vmax( pInfo->m_Values[i].m_Average, info.m_HighAverage );
			info.m_LowAverage = vmin( pInfo->m_Values[i].m_Average, info.m_LowAverage );
		}

		info.m_HighValue = vmax( pInfo->m_Values[i].m_Value, info.m_HighValue );
		info.m_LowValue = vmin( pInfo->m_Values[i].m_Value, info.m_LowValue );
